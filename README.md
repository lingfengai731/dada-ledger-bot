...
- **Saved 回执**:默认存好后**直接给原提交消息点 ✅ 反应**(不再发一条文字,少一条消息、降低封号足迹;`CONFIRM_WITH_REACTION` 控制)。反应贴不上或 preview 模式则回退到 `✅ Saved to Notion.` 文字并**引用**原消息(点一下跳回去,`ok all` 时每笔各一条)。
...
- **发错了怎么删**:引用发错的原消息(或机器人的确认摘要),回一句含 `cancel` 的话(如 `cancel unsaved expenses`)即可只删那一笔;单发 `cancel` 删最近一笔;`cancel all` 取消你名下全部待确认。
...
CONFIRM_WITH_REACTION=true                    # 存好后给原提交消息点 ✅ 反应代替发一条"Saved"文字(少发一条消息,降低足迹);preview 或反应贴不上则自动回退文字;false 关闭
