...
PAIRING_CODE_REFRESH_MS=20000                # 未登录时配对码刷新节流;默认 20 秒,不要低于 15 秒
...
3. 一条命令把二维码发布成网页(脚本会每秒同步最新二维码和 pairing code):
...
- 如果图片带 caption 且 caption 像报账格式,即使图片下载失败也会按**caption 文字**继续确认,避免静默丢单;确认摘要会明确提示"图片已发但 WhatsApp Web 下载失败",不会再误写成 "No photo attached"。
