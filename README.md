...
群里收到 [收据照片(+caption)] 和/或 [文字说明]
      │   照片的 caption 直接当文字说明读;图文可一条发、也可分两条发
...
- **未确认兜底**:草稿超过 **30 分钟**没回 `ok` → @发件人提醒;超过 **8 小时** → 信息齐全的自动写入(`sweepPending`)。
...
- [x] 确认后写入 Notion AUTO-LEDGER + 本地 SQLite
- [x] 群内自然语言问答(`/ask` `/total`),入群三语自我介绍
- [x] **婚礼日程表智能补全 + 场地优先 + 上下文记忆 + 巴厘岛时间**
- [x] **强制规则:婚期/PIC 缺失显示 `???` 并拒绝保存**
- [x] 部署新加坡 VPS,7×24 在线;婚礼日程表**实时**读取
- [x] PDF 发票识别;PIC/HANDLER 选项实时同步;重复/金额异常/糊图防呆;`/undo` 撤销
- [x] **标准笔记格式 `wed`/`pic` 标签**;发票日期优先从收据图读
- [x] **照片 caption 直接当说明**;**智能自适应收集窗口**(一条发全的更快)
- [x] **报销**:转账截图 → `REIMBURSED` 列(一图多笔、人名进 HANDLER)
- [x] **收据原图附到 Notion 行**;`/iam` 自学发件人→员工映射
- [x] **30 分钟 @ 提醒 + 8 小时自动写入**兜底;**每晚明细账单**私发老板
- [x] 待确认时**忽略闲聊**(不刷屏);vendor/description 自动去重精简
